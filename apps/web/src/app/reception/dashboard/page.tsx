...
export default function ReceptionDashboardPage() {
    ...
    if (phoneVal.length === 10) {
      try {
        ...
        const res = await fetch(
          `${BACKEND_URL}/patients?search=${encodeURIComponent(phoneVal)}`,
          {
            headers: {
              'Content-Type': 'application/json',
              ...(token
    ...
    try {
      ...
      if (!pId) {
        const searchRes = await fetch(
          `${BACKEND_URL}/patients?search=${encodeURIComponent(
            walkinForm.phone,
          )}`,
          {
            headers,
        ...
        if (searchRes.ok) {
          ...
          const found = Array.isArray(list)
            ? list.find(
                (p: any) => p.phone === walkinForm.phone,
              )
            : null;
        ...
        if (!pId) {
          ...
          if (patRes.ok) {
            ...
            const errBody = await patRes
              .json()
              .catch(() => ({}));

            throw new Error(
              errBody.message ||
                'Failed to register patient record.',
            );
          }
        }
      }
      ...
      showToast(
        err.message ||
          'Network error while issuing token.',
        'error',
      );
    } finally {
  ...
  const openBilling = async (apt: any) => {
    ...
    const consult =
      Number(apt.doctor?.consultationFee) || 500;
  ...
  const updateBill = (field: string, val: number) => {
    setBill((prev) => {
      ...
      const sub =
        Number(next.consult) +
        Number(next.lab) +
        Number(next.treatment) +
        Number(next.pharma);

      next.net = Math.max(
        0,
        sub - Number(next.discount),
      );
  ...
  const finalizePayment = async (mode: string) => {
    ...
    try {
      ...
      if (mode.includes('Cash')) {
        if (!patientId) {
          throw new Error(
            'Patient information is missing for settlement.',
          );
        }

        const settleRes = await fetch(
          `${BACKEND_URL}/billing/consolidated/${patientId}/settle`,
          {
            method: 'POST',
            headers,
            credentials: 'include',
        ...
        const settleData = await settleRes
          .json()
          .catch(() => ({}));
        ...
        if (!settleRes.ok) {
          throw new Error(
            settleData.message ||
              'Cash settlement failed on server.',
          );
        }
      ...
      if (!orderRes.ok || !orderData.success) {
        throw new Error(
          orderData.message ||
            'Failed to create secure payment order',
        );
      }
      ...
      const options = {
        ...
        handler: async function (response: any) {
          try {
            ...
            showToast(
              err.message ||
                'Error connecting during verification',
              'error',
            );
          } finally {
            setPaying(false);
      ...
      showToast(
        err.message ||
          'Could not initialize payment settlement.',
        'error',
      );
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#0b0f19] font-sans text-slate-900 dark:text-slate-100 p-4 md:p-8 max-w-7xl mx-auto space-y-6 relative overflow-hidden transition-colors">
      ...
      {liveAlert && (
        <div className="p-3.5 bg-emerald-600 text-white text-xs rounded-2xl font-bold flex justify-between items-center shadow-xl animate-bounce border border-emerald-400/30">
          ...
          <button
            ...
            className="bg-white/20 px-2.5 py-0.5 rounded-lg cursor-pointer"
          >
            ✕
          </button>
        </div>
      ...
      <div className="bg-white dark:bg-[#111827] backdrop-blur-xl p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-xl flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-blue-600 dark:text-blue-400">
              Reception & Discharge Terminal
            </span>
          </div>

          <h1 className="text-2xl font-black text-slate-900 dark:text-white mt-1">
            Live Counter & Billing Queue
          </h1>
        </div>
        ...
        <div className="flex gap-2">
          <button
            onClick={() => fetchData()}
            className="px-4 py-2.5 bg-slate-100 dark:bg-[#1f2937] hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-xl text-xs font-bold border border-slate-300 dark:border-slate-700 transition shadow-xs cursor-pointer"
          >
            🔄 Refresh
          </button>
          ...
          <button
            ...
            className="px-4 py-2.5 bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 dark:hover:bg-rose-900/60 text-rose-600 dark:text-rose-400 rounded-xl text-xs font-bold border border-rose-200 dark:border-rose-900 transition shadow-xs cursor-pointer"
          >
            Logout
          </button>
        </div>
        ...
        <div className="lg:col-span-4 bg-white dark:bg-[#111827] backdrop-blur-xl p-6 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl space-y-4">
          ...
          <div className="border-b border-slate-100 dark:border-slate-800 pb-3">
            ...
            <p className="text-[11px] text-slate-400 mt-0.5">
              Instant counter registration & queue assignment
            </p>
          </div>
                ...
                {loading ? (
                  ...
                  list.map((a) => {
                    return (
                      <tr
                        key={a.id}
                        className="hover:bg-slate-50 dark:hover:bg-[#1f2937]/50 transition"
                      >
                        ...
                        <td className="py-3.5 px-2 text-slate-500 dark:text-slate-400 font-mono">
                          {a.patient?.phone ||
                            'N/A'}
                        </td>
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-start border-b border-slate-100 dark:border-slate-800 pb-4">
                ...
                <button
                  ...
                  className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 hover:bg-slate-200 dark:hover:bg-slate-700 hover:text-slate-900 dark:hover:text-white flex items-center justify-center font-bold transition-colors cursor-pointer"
                >
                  ✕
                </button>
              </div>
                ...
                <div className="flex justify-between items-center gap-3 p-3 bg-slate-50 dark:bg-[#1f2937] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xs">
                  ...
                  <div className="relative w-24 shrink-0">
                    ...
                    <input
                      ...
                      className="w-full p-2.5 pl-6 bg-white dark:bg-[#111827] border border-slate-200 dark:border-slate-700 rounded-xl font-semibold text-right text-slate-800 dark:text-white outline-none focus:border-blue-500"
                    />
                  </div>
                </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#111827] text-slate-900 dark:text-slate-100 rounded-3xl max-w-sm w-full p-6 shadow-2xl space-y-4 animate-in zoom-in-95 border border-slate-100 dark:border-slate-800">
            ...
            <div className="text-center space-y-1 border-b border-slate-100 dark:border-slate-800 pb-3">
              ...
              <span className="w-10 h-10 rounded-full bg-emerald-100 dark:bg-emerald-950 text-emerald-600 dark:text-emerald-400 inline-flex items-center justify-center font-bold text-base shadow-xs">
                ✓
              </span>
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="flex-1 py-3 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-2xl font-bold text-xs transition cursor-pointer"
              >
                🖨️ Print Receipt
              </button>
              ...
              <button
                ...
                className="px-4 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl font-bold text-xs transition cursor-pointer shadow-md shadow-blue-600/20"
              >
                Close
              </button>
            </div>
