    ...
    <div className="min-h-screen bg-slate-50 p-6 md:p-10 font-sans">
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center gap-3">
          <Link href="/admin/billing" className="w-9 h-9 rounded-xl bg-white border border-slate-200 hover:bg-slate-100 flex items-center justify-center text-slate-700 text-xs font-bold transition shadow-sm">
            ←
          </Link>
          <div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="text-[11px] font-bold text-slate-600 uppercase">Select Patient <span className="text-rose-500">*</span></label>
                <select required value={patientId} onChange={(e) => setPatientId(e.target.value)} className="w-full mt-1.5 p-3 text-xs border border-slate-200 rounded-xl bg-slate-50 font-bold text-slate-800 outline-none focus:border-blue-500">
              ...
              <div>
                ...
                <select value={doctorId} onChange={(e) => handleDoctorChange(e.target.value)} className="w-full mt-1.5 p-3 text-xs border border-slate-200 rounded-xl bg-slate-50 font-medium text-slate-800 outline-none focus:border-blue-500">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div>
                ...
                <input type="number" min="0" value={consultationFee} onChange={(e) => setConsultationFee(Number(e.target.value))} className="w-full mt-1.5 p-3 text-xs border border-slate-200 rounded-xl font-mono font-bold text-slate-800 outline-none focus:border-blue-500" />
              </div>

              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase">Treatment / Procedures</label>
                <input type="number" min="0" value={treatmentCharges} onChange={(e) => setTreatmentCharges(Number(e.target.value))} className="w-full mt-1.5 p-3 text-xs border border-slate-200 rounded-xl font-mono font-bold text-slate-800 outline-none focus:border-blue-500" />
              </div>

              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase">Pharmacy / Medicines</label>
                <input type="number" min="0" value={medicineCharges} onChange={(e) => setMedicineCharges(Number(e.target.value))} className="w-full mt-1.5 p-3 text-xs border border-slate-200 rounded-xl font-mono font-bold text-slate-800 outline-none focus:border-blue-500" />
              </div>

              <div>
                <label className="text-[10px] font-bold text-slate-500 uppercase">Discount Concession</label>
                <input type="number" min="0" value={discount} onChange={(e) => setDiscount(Number(e.target.value))} className="w-full mt-1.5 p-3 text-xs border border-slate-200 rounded-xl font-mono font-bold text-rose-600 outline-none focus:border-rose-500" />
              </div>
            </div>

            <div className="p-4 bg-white text-slate-900 rounded-xl flex items-center justify-between border border-slate-100 shadow-sm">
              <div>
              ...
              <div className="text-2xl font-black font-mono text-emerald-600">₹{grandTotal.toFixed(2)}</div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <select value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)} className="w-full mt-1.5 p-3 text-xs border border-slate-200 rounded-xl bg-slate-50 font-bold text-slate-800 outline-none focus:border-blue-500">
              ...
              <div>
                ...
                <select value={paymentStatus} onChange={(e) => setPaymentStatus(e.target.value)} className="w-full mt-1.5 p-3 text-xs border border-slate-200 rounded-xl bg-slate-50 font-bold text-slate-800 outline-none focus:border-blue-500">
              ...
              <div className="md:col-span-2">
                ...
                <input type="text" placeholder="e.g. Counter A Reception Receipt" value={notes} onChange={(e) => setNotes(e.target.value)} className="w-full mt-1.5 p-3 text-xs border border-slate-200 rounded-xl outline-none focus:border-blue-500" />
              </div>
            </div>
          </div>

          <div className="flex justify-end gap-3">
            <button type="button" onClick={() => router.push('/admin/billing')} className="px-5 py-2.5 bg-white border border-slate-200 text-slate-600 hover:bg-slate-100 rounded-xl text-xs font-bold transition shadow-sm">
              Cancel
            </button>
            <button type="submit" disabled={submitting} className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 disabled:bg-emerald-300 text-white rounded-xl text-xs font-bold transition shadow-md flex items-center gap-2">
