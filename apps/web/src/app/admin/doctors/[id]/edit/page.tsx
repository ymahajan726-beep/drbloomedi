...
export default function EditDoctorPage() {
  ...
  useEffect(() => {
    async function loadDoctorAndDepts() {
      try {
        setLoading(true);
        const [docRes, deptsRes] = await Promise.all([
          fetch(`http://localhost:4000/doctors/${doctorId}`),
  ...
  if (loading) {
    return <div className="p-8 text-center text-slate-400 text-xs">Loading doctor details...</div>;
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 font-sans">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-slate-900">Edit Doctor Profile</h1>
          <p className="text-xs text-slate-500 mt-1">Update clinical wing, phone or qualification</p>
        </div>
        <Link href="/admin/doctors" className="text-xs font-semibold text-slate-500 hover:text-slate-800">
          ← Back to Doctors
        </Link>
      </div>
      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-600 mb-1">Department Wing</label>
              <select
