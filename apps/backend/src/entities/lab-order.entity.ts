...
export class LabOrder {
  ...
  @JoinColumn({ name: 'doctorId' }) doctor: Doctor;

  @ManyToOne(() => LabTest, { onDelete: 'RESTRICT' })
  @JoinColumn({ name: 'labTestId' }) labTest: LabTest;

  @Column({ type: 'enum', enum: LabOrderStatus, default: LabOrderStatus.ORDERED }) status: LabOrderStatus;

  @Column({ nullable: true }) resultValue: string;
  @Column({ type: 'text', nullable: true }) technicianRemarks: string;
  @Column({ type: 'boolean', default: false }) isAbnormal: boolean;
  @Column({ type: 'decimal', precision: 10, scale: 2, default: 0.0 }) billedAmount: number;
  @Column({ type: 'text', nullable: true }) reportFileUrl: string;
