...
export class PaymentService {
  ...
  constructor(private readonly dataSource: DataSource) {
    if (!/^rzp_(test|live)_[A-Za-z0-9]+$/.test(this.razorpayKeyId)) {
      throw new Error(
        'RAZORPAY_KEY_ID must be a valid Razorpay key starting with rzp_test_ or rzp_live_',
      );
    }
  ...
  async createOrder(billId: string, customAmount?: number) {
    ...
    const order = await this.razorpay.orders.create({
      ...
      currency: 'INR',
      receipt: billId
        ? `bill_${billId}`.slice(0, 40)
        : `receipt_${Date.now()}`,
      notes: billId ? { billId } : undefined,
    });

    this.logger.log(
      `Created Razorpay order ${order.id} for target reference: ${billId}`,
    );
  ...
  async verifyPayment(body: {
    ...
    if (
      ...
      this.logger.warn(
        `Cryptographic signature mismatch for payment ID: ${body.razorpay_payment_id}`,
      );
      throw new BadRequestException(
    ...
    if (
      ...
      throw new BadRequestException(
        `Razorpay payment is not payable: ${payment.status}`,
      );
    }
    ...
    if (targetId) {
      try {
        ...
        this.logger.warn(
          'Could not persist appointment status:',
          e,
        );
      }
    }
    ...
    try {
      ...
      if (bill) {
        ...
        this.logger.log(
          `Billing record ${bill.id} successfully updated to PAID.`,
        );
      } else {
        ...
        this.logger.log(
          `New PAID billing record created automatically for amount ₹${paidAmount}.`,
        );
      }
    } catch (e) {
      this.logger.error(
        'Failed to persist billing record during verification:',
        e,
      );
    }
  ...
  async handleWebhook(
    ...
    if (!signature || !rawBody) {
      throw new BadRequestException(
        'Razorpay webhook signature and raw body are required',
      );
    }
